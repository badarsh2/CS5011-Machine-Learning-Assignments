"""
Program to train a 3 layered NN
"""

from numpy import genfromtxt, min, max, divide, shape, tile, exp, multiply, unique, sqrt, sum, exp, dot, uint8, argmax
...
class NeuralNetwork:
	...
	def train_nn(self, X, Y, n_iter, alpha):
	    """
...
def transform(x, min_val=None, ranges=None):
    ...
    if ranges is None:
        ranges = max(x - tile(min_val, (shape(x)[0], 1)), axis=0)
...
def main():
	...
	# Centering the data and normalizing to unit variance
	...
	# Training a NN and backpropagating
	network = NeuralNetwork(nn_input, nn_hidden, nn_output)
	network.train_nn(train_X, train_Y, 10000, alpha)
